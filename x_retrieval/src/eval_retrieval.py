...
class EvalRunner:
    ...
    def build_corpus_index(self):
        ...
        if self.save_index:
            if hasattr(self.args, model_eval_dir):
        ...
        all_ctx_vector = []
        for mini_batch in tqdm(range(0, len(self.eval_dataset.candidates), self.args.per_gpu_eval_batch_size)):
            contexts = self.eval_dataset.candidates[mini_batch:mini_batch + self.args.per_gpu_eval_batch_size]
    ...
    def inference(self, steps=0):
        ...
        with open(f'{self.args.output_dir}/result_{steps}.json', 'w') as f:
            json.dump(result_dict, f)
        self.measure_result(result_dict, meters)
        with open(f'{self.args.output_dir}/performance_{steps}.json', 'w') as f:
            eval_res = {field_name: meter.avg for field_name, meter in meters.items()}
            json.dump(eval_res, f)
        self.eval_first_hop(result_dict, steps)

        return eval_res

    def eval_first_hop(self, res_dict_or_path, steps):
        ...
        if isinstance(res_dict_or_path, str):
            ...
            _save_path = Path(res_dict_or_path).parent / f"first_hop_performance_{steps}.json"
        elif isinstance(res_dict_or_path, Dict):
            res_dict = res_dict_or_path
            _save_path = f'{self.args.output_dir}/first_hop_performance_{steps}.json'
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--corpus_dataset_path", type=str, default="x_retrieval/datasets/mdr_hotpot/mdr_corpus.jsonl")
    parser.add_argument("--eval_dataset_path", type=str, default="x_retrieval/datasets/mdr_hotpot/retrieval_eval.jsonl")
    parser.add_argument("--model_path", type=str, default="downlaod/eval_models/mdr_mix" )
    parser.add_argument("--output_dir", type=str, default="x_retrieval/eval_output")
    parser.add_argument("--per_gpu_eval_batch_size", type=int, default=64, help="Batch size for the passage encoder forward pass")
    parser.add_argument("--if_add_prefix", type=bool, default=True)
    parser.add_argument("--query_length", type=int, default=50)
    ...
    eval_runner = EvalRunner(model_ckpt_or_path=opt.model_path,
