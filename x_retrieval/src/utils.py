...
def save_pickle(my_path, my_data):
    with open(str(my_path), "wb") as fIn:
        pickle.dump(my_data, fIn)

def load_pickle(my_path):
    with open(str(my_path), "rb") as fOut:
        return pickle.load(fOut)

def calculate_time(fct):
    @functools.wraps(fct)
    def wrapper(*args, **kwargs):
        ...
        print(f"Function {fct.__name__} cost: {execution_time} s")
        return result
    return wrapper